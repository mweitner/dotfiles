...
| command | description |
|---------|-------------|
| `Ctrl-Space` | $mod (leader key)  /  default: Ctrl-b |

tmux has 2 modes **default** and **copy** mode. Default mode is like vi's insert mode. In  copy mode text from tmux
windows can be copied and pasted anywhere. It supports copy of non visual text by select and scroll (see Control
section).
...
| split vertical | cut through vertical line from left to right  /  - split window top/bottom |     |
| horizontal | horizontal line from left to right |     |
| split horizontal | cut through horizontal line from top to bottom  /  - split window left/right |     |

## Control

| command | description |
|---------|-------------|
| `$mod :` | opens command line mode e.g.  /  -`:resize-pane -D 5`to resize pane down by 5  /  -`:splitw -fh`to split horizontal (left/right) through full window height  /  Alternative the command following`: <command>`  /  - can be executed at terminal with`tmux` |
| `Ctrl-d` or "Prompt> `kill-window`" | kill selected **pane**:  /  - If last pane, kill**window**  /  - if last window, kill**session**  /  - if last session, kill**tmux server** |
| `$mod r` | reload tmux config |
| `$mod Shift+I` | tmux environment reload and plugin initialization (plugin fetch and source)  /  tmux says`TMUX environment reloaded …`  /  `q` - quit plugin load dialog (**not Enter** as the dialog says) |
| `$mode [` | switch from default to copy mode.  /  **Copy tmux terminal text:**  /  -`v` or `V`: start selection  /  -`y`: yank (copy). As we defined `xsel --clipboard`at tmux config it copies into clipboard.  /  - Paste from clipboard anywhere else within your system  /  **Navigate while in copy mode:**  /  - Ctrl+u - scroll up  /  - Ctrl+d - scroll down  /  - \\ - search  /  q - exit copy mode back to default mode |
...
| `tmux move-window -t <target> -s <source>:<id>` | move window with `id` from `source` session to `target` session. Example: `tmux move-window -t term-ws1 -s term-dev:5` |
...
| command | description |
|---------|-------------|
| *$mod -* | split **vertical**(up/down)  /  split through**f**ull window width:  /  *:splitw -fv* |
| *$mode \|* | split **horizontal**(left/right)  /  split through**f**ull window height:  /  *:splitw -fh* |
...
| `$mod Alt+[1,2,3,4,5]`  |             |
| `$mod o` | cycle selected pane clockwise  /  Hint:**o**ther |
...
| command | description |
|---------|-------------|
| `$mod [` | `/`- input search regex downward  /  `?` - input search regex upward |
...
| command | description |
|---------|-------------|
| $mod /  | input regex search expression  /  `Enter`- copy highlighted search result (default)  /  in our case:`\ y` (see SpaceVim) |
...
| command | description |
|---------|-------------|
| tmuxp freeze | freeze current tmux session by it's name at config path e.g  /  `~/.config/tmuxp/name.yml`. |

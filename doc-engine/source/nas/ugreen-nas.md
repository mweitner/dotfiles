...
1. `scp: dest open ... No such file or directory` (on NAS uploads)
...
1. `Certificate verification failed: The certificate is NOT trusted` (apt)
...
1. `Ign: ... deb.debian.org ...` / apt hangs at `0% [Working]`
...
1. `E: Unable to locate package git` after TLS errors
...
1. `You might want to run 'apt --fix-broken install'`

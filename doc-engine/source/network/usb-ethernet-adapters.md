...
Your machine-network use cases (crane, concrete, mining, LPO) often involve simultaneous connections to incompatible
networks. Without isolation, DHCP and routing would conflict. Multiple USB Ethernet adapters solve this by providing
independent IP stacks per network:
...
1. Run `nmcli connection show | grep '^Machine-'` to list created profiles.
2. Activate profile manually:
...
- If adapter shows but with different MAC, update the registry in `~/.local/bin/setup-adapters` or use
  `setup-machine-network-profiles` directly with `--usb-mac <MAC>`.
...
1. Run `setup-adapters --list-adapters` to confirm.
2. Use the new adapter-d in your setup commands.

Alternatively, keep the adapter registry in `~/dotfiles/shell/setup-adapters.sh` and re-run `install-fedora.sh Phase 2`
to re-symlink.

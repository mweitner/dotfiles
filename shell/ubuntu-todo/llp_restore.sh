...
#
# llp_restore.sh - restore llp project to recover from previous git clean -xdf
#
...
if [[ ! -d "${llp_backup_dir}" ]]; then
  echo "[llp_backup] Error no backup folder to recover from"
  return 253
fi
...
pushd "${llp_system_test_root}"
git clean -xdf
popd

#restore system-test project root 
if [[ -d "${llp_backup_dir}/.idea" ]]; then
  cp -R "${llp_backup_dir}/.idea" "${llp_system_test_root}"
fi
if [[ -d "${llp_backup_dir}/.run" ]]; then
  cp -R "${llp_backup_dir}/.run" "${llp_system_test_root}"
fi
if [[ -d "${llp_backup_dir}/config" ]]; then
  cp -R "${llp_backup_dir}/config" "${llp_system_test_root}"
fi
